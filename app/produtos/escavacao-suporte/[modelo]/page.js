...
export function generateStaticParams() {
  return EXC_MODELS.map((m) => ({ modelo: m.slug }));
}

export function generateMetadata({ params }) {
  const m = EXC_MODELS.find((x) => x.slug === params.modelo);
  if (!m) return { title: 'Equipamento' };
  return { title: m.title, description: m.summary };
}

export default function ExcModel({ params }) {
  const m = EXC_MODELS.find((x) => x.slug === params.modelo);
  if (!m) notFound();

  const groupLabel = EXC_GROUPS.find((x) => x.key === m.group)?.label || m.group;
  const others = EXC_MODELS.filter((x) => x.slug !== m.slug && x.group === m.group).slice(0, 3);
  ...
  const specs =
    m.specs && m.specs.length
      ...
      : [
          ['Grupo', groupLabel],
          ['Categoria', 'Escavação convencional (NATM)'],
  ...
  return (
    <>
      <section className="phero">
        <div className="wrap">
          <div className="crumb">
            ...
            <Link href="/produtos/escavacao-suporte">Escavação convencional</Link> / {m.title}
          </div>
          <h1>{m.title}</h1>
          <p>{m.summary}</p>
        </div>
        ...
        <section className="dark2">
          <div className="wrap">
            ...
            <div className="case-grid" style={{ marginTop: '2rem' }}>
              {others.map((o) => (
                <Link href={`/produtos/escavacao-suporte/${o.slug}/`} className="case" key={o.slug}>
      ...
      <section className="dark2" style={{ paddingTop: others.length ? 0 : undefined }}>
        <div className="wrap">
          <Link href="/produtos/escavacao-suporte" className="btn ghost">← Voltar para Escavação convencional</Link>
        </div>
      </section>
