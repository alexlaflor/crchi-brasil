...
import { AGRO_MODELS, AGRO_GROUPS } from '@/lib/agricolas';

const BASE = process.env.NEXT_PUBLIC_BASE_PATH || '';

// Gera uma página estática para cada equipamento (necessário para output: 'export')
export function generateStaticParams() {
  return AGRO_MODELS.map((m) => ({ modelo: m.slug }));
}

export function generateMetadata({ params }) {
  const m = AGRO_MODELS.find((x) => x.slug === params.modelo);
  if (!m) return { title: 'Equipamento' };
  return { title: m.title, description: m.summary };
}

export default function AgroModel({ params }) {
  const m = AGRO_MODELS.find((x) => x.slug === params.modelo);
  if (!m) notFound();

  const groupLabel = AGRO_GROUPS.find((x) => x.key === m.group)?.label || m.group;
  const others = AGRO_MODELS.filter((x) => x.slug !== m.slug && x.group === m.group).slice(0, 3);
  ...
  return (
    <>
      <section className="phero">
        <div className="wrap">
          <div className="crumb">
            ...
            <Link href="/produtos/maquinas-agricolas">Máquinas agrícolas</Link> / {m.title}
          </div>
          <h1>{m.title}</h1>
          <p>{m.summary}</p>
        </div>
            ...
            <div>
              ...
              {m.description && m.description.length ? (
                ...
                <p style={{ color: '#c9c6be' }}>
                  Espaço reservado para a descrição completa do equipamento: aplicação, potência,
                  capacidade, diferenciais técnicos e casos de uso. Conteúdo genérico — preencha com
                  as informações oficiais quando disponíveis.
                </p>
              )}
            </div>
            <dl className="specs">
        ...
        <section className="dark2">
          <div className="wrap">
            ...
            <div className="case-grid" style={{ marginTop: '2rem' }}>
              {others.map((o) => (
                <Link href={`/produtos/maquinas-agricolas/${o.slug}/`} className="case" key={o.slug}>
      ...
      <section className="dark2" style={{ paddingTop: others.length ? 0 : undefined }}>
        <div className="wrap">
          <Link href="/produtos/maquinas-agricolas" className="btn ghost">← Voltar para Máquinas agrícolas</Link>
        </div>
      </section>
      ...
      <section className="cta">
        <div className="wrap">
          ...
          <p>Envie os dados da sua operação e retornamos com a configuração recomendada.</p>
