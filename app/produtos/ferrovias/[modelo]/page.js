...
// Gera uma página estática para cada produto (necessário para output: 'export')
export function generateStaticParams() {
  return FER_MODELS.map((m) => ({ modelo: m.slug }));
}

export function generateMetadata({ params }) {
  const m = FER_MODELS.find((x) => x.slug === params.modelo);
  if (!m) return { title: 'Produto' };
  return { title: m.title, description: m.summary };
}

export default function FerModel({ params }) {
  const m = FER_MODELS.find((x) => x.slug === params.modelo);
  if (!m) notFound();

  const groupLabel = FER_GROUPS.find((x) => x.key === m.group)?.label || m.group;
  const others = FER_MODELS.filter((x) => x.slug !== m.slug && x.group === m.group).slice(0, 3);
  ...
  return (
    <>
      <section className="phero">
        <div className="wrap">
          <div className="crumb">
            ...
            <Link href="/produtos/ferrovias">Ferrovias</Link> / {m.title}
          </div>
          <h1>{m.title}</h1>
          <p>{m.summary}</p>
        </div>
            ...
            <div>
              ...
              {m.description && m.description.length ? (
                ...
                <p style={{ color: '#c9c6be' }}>
                  Espaço reservado para a descrição completa do produto: aplicação, bitola, norma
                  atendida, diferenciais técnicos e projetos de referência. Preencha com as
                  informações oficiais quando disponíveis.
                </p>
              )}
            </div>
            <dl className="specs">
        ...
        <section className="dark2">
          <div className="wrap">
            ...
            <div className="case-grid" style={{ marginTop: '2rem' }}>
              {others.map((o) => (
                <Link href={`/produtos/ferrovias/${o.slug}/`} className="case" key={o.slug}>
      ...
      <section className="dark2" style={{ paddingTop: others.length ? 0 : undefined }}>
        <div className="wrap">
          <Link href="/produtos/ferrovias" className="btn ghost">← Voltar para Ferrovias</Link>
        </div>
      </section>
