...
export default function MineracaoFilter() {
  const [group, setGroup] = useState('all');
  const filtered = group === 'all' ? MIN_MODELS : MIN_MODELS.filter((m) => m.group === group);
    ...
    <>
      ...
      <div className="case-grid">
        {filtered.map((m) => {
          ...
          return (
            <Link href={`/produtos/mineracao/${m.slug}/`} className="case" key={m.slug}>
              <div className="ph">
